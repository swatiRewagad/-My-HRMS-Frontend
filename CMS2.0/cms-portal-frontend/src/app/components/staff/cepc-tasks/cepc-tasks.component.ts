...
@Component({
    ...
    <div class="tasks-container">
      <header class="topbar">
        <div class="topbar-left">
          ...
          <h2>CEPC - {{ isHistoryView() ? 'Completed' : 'My Tasks' }}</h2>
        </div>
  ...
  styles: [`
    ...
    .topbar {
      background: #0d47a1; color: white; padding: 12px 24px;
    ...
    .stat-card { background: white; padding: 16px; border-radius: 8px; text-align: center; border-left: 4px solid #0d47a1; }
    ...
    .task-card:hover { border-color: #0d47a1; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
    .task-card.overdue { border-left: 4px solid #d32f2f; }
    .task-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
    .complaint-num { font-weight: 600; color: #0d47a1; font-size: 14px; }
...
export class CepcTasksComponent implements OnInit {
  ...
  openTask(task: ComplaintTask) {
    this.router.navigate(['/staff/cepc/task', task.complaintNumber]);
  }
